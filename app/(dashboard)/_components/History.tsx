...
function History({ userSettings }: Props) {
  ...
  const historyDataQuery = useQuery({
    ...
    queryFn: () =>
      fetch(`/api/history-data?timeframe=${timeFrame}&year=${period.year}&month=${period.month}`).then(
        (res) => res.json()
      ),
  });
  ...
  return (
    <div className="container">
      <h2 className="mt-12 text-3xl font-bold"></h2>
          ...
          <SkeletonWrapper isLoading={historyDataQuery.isLoading}>
            ...
            {!dataAvailable && (
              <Card className="flex h-[300px] flex-col items-center justify-center bg-background">
                ...
                <p className="text-sm text-muted-foreground">Hãy chọn thời gian khác hoặc thêm thu chi</p>
              </Card>
            )}
          </SkeletonWrapper>
        </CardContent>
